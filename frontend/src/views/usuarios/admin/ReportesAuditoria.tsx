...
export const ReportesAuditoria: React.FC = () => {
  ...
  const [reportes, setReportes] = useState<ReporteInscripcion[]>([]);
  ...
  const cargarReportes = useCallback(() => {
    ...
    Promise.all([
      ...
      .then(([cRes, iRes]) => {
        const carrerasData = cRes.data;
        setCarreras(carrerasData);

        const inscripciones = iRes.data;
        const conteoPorCarrera = new Map<string, number>();
        for (const ins of inscripciones) {
          const carreraNombre = ins.Asignaciones?.Cursos?.Carreras?.nombre;
          if (carreraNombre) {
            conteoPorCarrera.set(carreraNombre, (conteoPorCarrera.get(carreraNombre) ?? 0) + 1);
          }
        }

        const reportesData: ReporteInscripcion[] = carrerasData.map((c, i) => ({
          id: `r-${c.id_carrera}`,
          carrera: c.nombre,
          codigoPensum: c.id_carrera.toString().padStart(3, '0'),
          inscritosActivos: conteoPorCarrera.get(c.nombre) ?? 0,
          proyeccionMeta: 150,
          facultad: 'General',
        }));
  ...
  const totalProyeccion = reportes.reduce((acc, r) => acc + r.proyeccionMeta, 0);
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className={`p-4 rounded-2xl border ${styles.border} ${styles.panel} ${styles.shadow} flex items-center space-x-4`}>
          ...
          <div>
            ...
            <h4 className="text-xl font-bold text-slate-800">{loading ? '...' : totalInscritosGlobales} Alumnos</h4>
          </div>
        </div>
        ...
        <div className={`p-4 rounded-2xl border ${styles.border} ${styles.panel} ${styles.shadow} flex items-center space-x-4`}>
          ...
          <div>
            ...
            <h4 className="text-xl font-bold text-slate-800">
              {loading || totalProyeccion === 0 ? '...' : Math.round((totalInscritosGlobales / totalProyeccion) * 100)}% Alcance
            </h4>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white border border-gray-100 rounded-2xl shadow-xs overflow-hidden">
        ...
        {activeTab === 'reportes' && (
          <div className="overflow-x-auto">
            {loading ? (
              <div className="py-8 text-center text-gray-400 font-medium">Cargando reportes...</div>
            ) : (
