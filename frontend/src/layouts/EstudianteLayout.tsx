...
export const EstudianteLayout: React.FC<EstudianteLayoutProps> = ({ children, activeTab }) => {
  const { user, logout } = useAuth();

  // Menú lateral solicitado explícitamente para el estudiante
  const menuItems = [
    { id: 'tablero', label: 'Tablero (Inicio)', icon: <LayoutDashboard size={20} /> },
    { id: 'inscripciones', label: 'Inscripciones', icon: <BookOpen size={20} /> },
    { id: 'cursos', label: 'Cursos', icon: <FolderOpen size={20} /> },
    { id: 'expediente', label: 'Expediente', icon: <FileText size={20} /> },
    { id: 'mensajes', label: 'Mensajes / Avisos', icon: <MessageSquare size={20} /> },
    ...
    <div className="flex h-screen w-screen bg-[#f4f6f9] overflow-hidden">
      ...
      <aside className="w-64 bg-[#1a365d] flex flex-col shadow-xl">
        <div className="p-6 flex flex-col items-center">
          ...
          <p className="text-blue-200 text-[10px] uppercase tracking-widest font-bold">Portal Estudiante</p>
        </div>
        ...
        {/* PERFIL EN EL MENÚ (IGUAL AL ADMIN) */}
        <div className="p-4 border-t border-white/10">
          <div className="flex items-center space-x-3 p-2 bg-white/5 rounded-2xl mb-2">
            ...
            <div className="flex-1 overflow-hidden">
              <p className="text-white text-xs font-bold truncate">{user?.nombre || 'Alex Rivera'}</p>
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="h-20 bg-white border-b border-gray-100 flex items-center justify-between px-8 shrink-0">
          <div className="flex items-center space-x-4">
            <h1 className="text-xl font-black text-gray-800 uppercase tracking-tight">Panel Estudiante</h1>
